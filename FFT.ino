...
void audioSpectrum() {
  ...
  if (oldminiWindowMode != miniWindowMode) {  //  miniWindowMode selects btw off, low resolution, high resolution, mini osci. audio waterfall. Spectrums display , waterfall
    ...
    tft.fillRect(startX - 5, startY, 92, 25, TFT_BLACK);  // overwrite last window
  }
  ...
  if (audioMuted && !pressed) {
    ...
    if (!mStat) {
      mStat = true;
      tft.fillRect(startX, startY, 92, 25, TFT_BLACK);  // overwrite when muted
  ...
  if (miniWindowMode == 4) {  // audio waterfall horizontal
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        tft.drawPixel(startX + j, startY + (height - i), valueToWaterfallColor(gradient * wabuf[i][j]));
      }
    }
  }
...
void audioSpectrum256() {  // 512 bin audio watefrall, displays 240 channels after user inactivity
  ...
  while (digitalRead(ENC_PRESSED) == HIGH && !clw && !cclw && !get_Touch()) {
  ...
  free(framebuffer1);
  tft.fillRect(135, startY, 86, 25, TFT_BLACK);  // overwrite last miniwindow
...
void FFTSample(int sampleCount, int dly, bool drawOsci) {
  ...
  for (int i = 0; i < sampleCount; i++) {  // sampling loop
    ...
    if (drawOsci) {
      int am = sum / 100;  // for mini oscilloscope
      am = std::clamp(am, 43, 69);
      if (i < 86)                                     // Mini oscilloscope trace
        tft.drawPixel(135 + i, 250 + am, TFT_WHITE);  // use 250 instead of 293 removes the DC component ofthe pre
    
    }
    // Adjust and store the averaged value
    RvReal[i] = sum / (sampleCount / 64);
    RvImag[i] = 0;
  }
...
void audioFreqAnalyzer() {
  ...
  while (digitalRead(ENC_PRESSED) == HIGH && !clw && !cclw && !get_Touch()) {
    ...
    for (int i = 2; i < SAMPLES; i++) {
      tft.drawPixel(i * 1.90, y, valueToWaterfallColor((int)(RvReal[i])));
    }
