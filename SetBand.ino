...
void setBand(bool isWaterfall) {  // Use Select Band button to set start freq to a specific band, also used for range view
  ...
  while (true) {
    ...
    BandInfo& sel = bands[selected_band];
    ...
    if (strcmp(sel.bandName, "NEXT") == 0)
      ...
      if (!isWaterfall) {
        ...
        if (tinySACenterMode == false && tinySA_RF_Mode == true) {
          ...
          Serial.println(buffer);
          delay(100);
          Serial.println("rbw auto");  // Set resolution bandwidth to auto since span may have changed        
        }
#endif
        break;
      }
...
void autoloopBands() {
  ...
  if (selected_band == -1) {  // we have left the selected band, so change span back to default
    ...
    delay(100);
    Serial.println("rbw 12"); // set rbw back to default
