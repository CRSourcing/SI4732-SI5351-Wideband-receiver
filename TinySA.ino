void tinySAScreen() {  // Config tinySA through SecondScreen button
...
void readTSABtns() {
  ...
  while (1) {  // run a loop, do not automatically return to main menu
    ...
    /* tinySA programming via serial commands and scripts
    sweep
...
void synctinySA() {  // sync with tuned FREQ and extract signal strength of marker 1
  ...
  if (tinySA_RF_Mode == false) {  // no action in IF mode
    return;
  }
  ...
  if (syncEnabled) {
    ...
    tft.fillCircle(470, 20, 2, TFT_RED);
    delay(2);
    ...
    tft.fillCircle(470, 20, 2, TFT_BLACK);
    ...
    if (freqM1 > LOW_FREQ) {
      ...
      tft.fillCircle(470, 20, 2, TFT_GREEN);
...
void setSpanAndMarker() {  // centers the tinySA once when memo loaded or keypad frequency entry used
  char buffer[50];

  if (tinySACenterMode == false && tinySA_RF_Mode == true) {  // tinySA displays the selected band, but does not follow tuning
...
void loadIFMode() {  // load tinySA IF mode

  char buffer[50];
  Serial.println("load 1");  // preset 1
  delay(100);
  digitalWrite(tinySA_PIN, LOW);
  sprintf(buffer, "sweep center %ld", (long)(SI4735TUNED_FREQ * 1000));
  Serial.println(buffer);  // set the tinySA center to IF
  delay(100);
  sprintf(buffer, "sweep span %ld", span);
  Serial.println(buffer);  // change span to default
  delay(100);
  sprintf(buffer, "marker 2 %ld", (long)(SI4735TUNED_FREQ * 1000));
  delay(50);
  Serial.println("rbw 16");  // default resolution bandwidth
  preferences.putBool("tinySARFMode", false);
}
...
void convertToDbm(char* extractedSS) {
  ...
  if (t == 3) {
    ...
    char dBmString[9] = { 0 };
    char microVoltString[20] = { 0 };
    dBmValue = atof(extractedSS);

    snprintf(dBmString, sizeof(dBmString), "%3.0f dBm", dBmValue);

    float microVolts = pow(10, (dBmValue + 107) / 20);  // microvolts formula
    snprintf(microVoltString, sizeof(microVoltString), "%.1f uV", microVolts);

    tft.fillRect(288, 64, 50, 30, TFT_BLACK);
    tft.setTextColor(TFT_GREEN);
    tft.setCursor(288, 64);
    tft.setTextSize(1);
    if (dBmValue)
    tft.printf("%s", dBmString);
    tft.setTextSize(2);
...
void forceTSADefaults() {  //forces TSA to center when needed, or by pressing orange "R" button
